...
const Phone_Datas = ({ items }: any) => {
  ...
  return (
    <div className="like container m-auto bg-transparent p-8 pt-6 pb-6">
      <div className="card-like gap-8 grid cursor-pointer lg:grid-cols-5 md:grid-cols-3 grid-cols-1 my-6 bg-transparent ">
        {items &&
          items.map((item: any) => {
            return (
              <div
                className="card-body-like cursor-pointer rounded bg-white space-y-6 p-6 w-[100%] shadow-md text-center "
                key={item.id}
                onClick={() =>
